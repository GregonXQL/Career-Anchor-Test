...
施恩职业锚测评小程序后端。当前完成开发文档第 13 章的 M3：在 M2 测评闭环上补齐八种职业锚原创解析文案和 `/anchor-profiles` 接口，为完整报告页与历史报告提供数据。
...
当前 14 个单测覆盖 JWT、计分边界与同分优先级、邀请码状态与原子消耗、提交频率限制、报告换算、结果归属校验，以及解析文案 JSON 转换与固定排序。构建和测试均固定使用 Java 21。
